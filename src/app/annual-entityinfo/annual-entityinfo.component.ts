...
@Component({
  selector: 'app-annual-entityinfo',
  templateUrl: './annual-entityinfo.component.html',
  styleUrls: ['./annual-entityinfo.component.sass']
})
export class AnnualEntityinfoComponent implements OnInit {
  ...
  typeOfEntity: TypeOfEntity[] = [
    ...
    { value: 'plc_bvi', viewValue: 'BVI有限公司' },
  ...
  ngOnInit() {
    return this.orderService.getOrders()
      .subscribe( (orders) => {
        ...
        // 去除重复实体名字，去除空名字，去除bvi实体
        for (let i = 0; i < this.orderInfo.length; i++) {
          if (this.nameOfEntity.indexOf(this.orderInfo[i][0]) === -1 && this.orderInfo[i][0] !== null && this.orderInfo[i][2] !== 'plc_bvi') {
        ...
        this.orderService
          ...
          .subscribe((o) => {
            ...
            this.orderService
              ...
              .subscribe((ar_order) => {
                console.log(ar_order);
                this.orderId = ar_order.id;
                this.router.navigate(['/order/annual_renewal/', ar_order.id, 'introduction']);
                // this.orderService
                //   .updateEntity(entity)
                //   .subscribe(
                //     data => {
                //       this.router.navigate(['/order/annual_renewal/', o.id, 'introduction']);
                //     },
                //     error => {
