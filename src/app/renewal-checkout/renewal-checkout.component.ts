...
@Component({
  selector: 'app-renewal-checkout',
  templateUrl: './renewal-checkout.component.html',
  styleUrls: ['./renewal-checkout.component.sass']
})
export class RenewalCheckoutComponent implements OnInit {
  ...
  toNextStep() {
    ...
    this.orderService
      .generateIncorpPDF(id, 'annual_renewal_contract', sku_ids)
      .subscribe((letter: IncorpGeneratedDocument) => {
        generatingLetter = false;
        this.generating = generatingLetter;
      }, (error) => {
        generatingLetter = false;
        this.generating = generatingLetter;
