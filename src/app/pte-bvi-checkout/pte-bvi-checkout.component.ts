...
@Component({
  selector: 'app-pte-bvi-checkout',
  templateUrl: './pte-bvi-checkout.component.html',
  styleUrls: ['./pte-bvi-checkout.component.sass']
})
export class PteBviCheckoutComponent implements OnInit {
  ...
  toNextStep() {
    ...
    this.orderService
      .generateIncorpPDF(id, 'plc_bvi_basic_info', null)
      .subscribe((CLG: IncorpGeneratedDocument) => {
        ...
        generatingPLC = false;
        this.generating = generatingPLC || generatingLetter;
        console.error('Generated plc_bvi_basic_info PDF failed: ', error);
      });
